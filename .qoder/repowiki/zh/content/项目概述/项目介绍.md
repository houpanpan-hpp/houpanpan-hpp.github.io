...
G["posts/*.mdx<br/>文章"]
H["projects/*.mdx<br/>项目档案"]
I["reports/*.mdx<br/>评测报告"]
end
subgraph "业务逻辑(lib)"
J["posts.ts<br/>文章读取/解析/筛选"]
K["projects.ts<br/>项目读取/解析/排序"]
L["reports.ts<br/>报告读取/解析"]
M["utils.ts<br/>通用工具函数"]
end
subgraph "静态资源(public)"
N["search-index.json<br/>构建期生成"]
end
A --> E
A --> F
B --> F
...
## 核心组件
- 全局布局与元数据：定义站点标题、描述、关键词、Open Graph 与 Twitter 协议，统一页面风格与 SEO 基础信息。
...
- Hero：包含动态标签语轮播与粒子网络动画，营造沉浸式首屏体验。
